...
const List = () => {
  ...
  const [searchurl, setSearchurl] = useState("/api/hero-section");
    ...
    },
    {
      ...
      cell: (row) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link to={`/app/hero-section/edit/${row.id}`} className="btn">
  ...
  const deleteSection = async () => {
    try {
      await axios.delete(`/api/hero-section/${delrow}`);
      setOpen(false);
      setDelrow(null);
    } catch (error) {
    ...
    <>
      ...
      <Layout>
        <Grid container>
          <Grid item xs={1}></Grid>
          <Grid item xs={11}>
            <Link to="/app/hero-section/add" className="btn">
              <Button
                variant="contained"
                startIcon={<AddIcon />}
